...
ipcMain.handle("apple:nowplaying:monitor",async()=>{const s=get();const credential=loadCredential();if(!s.appleTvHost)throw new Error("Select an Apple TV first");if(!credential)throw new Error("Pair the Apple TV first");if(app.isPackaged)throw new Error("Companion monitor is currently available in developer mode only");const code=[
"import asyncio,json,pyatv,sys,base64,plistlib,time",
"def dec(v):",
" if isinstance(v,bytes):",
"  try:",
"   p=plistlib.loads(v)",
"   if isinstance(p,dict) and '$objects' in p and '$top' in p:",
"    o=p['$objects']",
"    def d(x,seen=None):",
"     seen=set() if seen is None else seen",
"     if isinstance(x,plistlib.UID):",
...
"      if i in seen:return '<cycle>'",
"      return d(o[i],seen|{i})",
"     if isinstance(x,dict):return {str(d(k,seen) if isinstance(k,plistlib.UID) else k):d(y,seen) for k,y in x.items() if k!='$class'}",
"     if isinstance(x,list):return [d(y,seen) for y in x]",
"     if isinstance(x,bytes):return '<bytes:%d>'%len(x)",
"     return x",
"    return d(p['$top'].get('root'))",
"  except Exception:return '<bytes:%d>'%len(v)",
" if isinstance(v,dict):return {str(k):dec(x) for k,x in v.items() if not any(z in str(k).lower() for z in ('credential','password','secret'))}",
" if isinstance(v,(list,tuple)):return [dec(x) for x in v]",
" if isinstance(v,(str,int,float,bool)) or v is None:return v",
" return str(v)",
"async def m():",
" loop=asyncio.get_running_loop(); cs=await pyatv.scan(loop,hosts=[sys.argv[1]]); c=cs[0]",
...
" a=await pyatv.connect(c,loop)",
" try:",
"  setup=a._protocol_handlers.get(pyatv.const.Protocol.Companion); api=None",
"  if setup:",
"   for x in getattr(setup,'interfaces',{}).values():",
"    q=getattr(x,'api',None)",
"    if q is not None and hasattr(q,'subscribe_event'):api=q;break",
"  if api is None:raise RuntimeError('CompanionAPI object not found')",
"  events=[]",
"  def cb_for(n):",
"   async def cb(data):events.append({'at':round(time.time(),3),'event':n,'data':dec(data)})",
"   return cb",
"  names=['NowPlayingInfo','_iMC','TVSystemStatus','SystemStatus']",
"  for n in names:",
"   try:api.listen_to(n,cb_for(n));await api.subscribe_event(n)",
"   except Exception as e:events.append({'event':n,'subscribe_error':type(e).__name__+': '+str(e)})",
"  snapshots=[]",
"  for i in range(4):",
"   snap={'second':i*5}",
"   for cmd in ('FetchCurrentNowPlayingInfoEvent','FetchUpNextInfoEvent'):",
"    try:snap[cmd]=dec(await api._send_command(cmd,{}))",
"    except Exception as e:snap[cmd]={'error':type(e).__name__+': '+str(e)}",
"   snapshots.append(snap)",
"   if i<3:await asyncio.sleep(5)",
"  await asyncio.sleep(3)",
"  print(json.dumps({'snapshots':snapshots,'events':events},ensure_ascii=False,indent=2))",
" finally:a.close()",
"asyncio.run(m())"
].join("\n");return await new Promise((resolve,reject)=>{const p=spawn(process.env.PYTHON_EXE||"python",["-X","utf8","-c",code,s.appleTvHost,credential],{windowsHide:true,stdio:["ignore","pipe","pipe"],env:{...process.env,PYTHONUTF8:"1",PYTHONIOENCODING:"utf-8"}});let out="",err="";p.stdout.on("data",d=>out+=d);p.stderr.on("data",d=>err+=d);p.on("error",reject);p.on("close",code=>code===0?resolve(redact(out).trim()):reject(new Error(redact(err||out||("Monitor exited "+code)).trim())))})});
